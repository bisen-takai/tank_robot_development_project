...
外部から受信する操作、設定、ファームウェア、その他の情報については、各入力を所有する機能が構文、長さ、シーケンス／リプレイ、対象、状態および機能上の意味を検証し、必要な暗号学的確認にはセキュリティ機能の結果を使用する。
...
起動・終了・再起動管理がアプリケーション起動結果、システム状態管理がBLE接続待機状態および通常操作可否を確定し、OTA更新管理がこれらの結果から`TRIAL_BOOT_VALIDATED`、`RECOVERY_SUCCEEDED`およびローカルOTAの最終結果を確定する。
