...
| 主電源操作部       | 電源部・制御部   | 常時給電側の回路（AON）＋GPIO通知             | 起動および通常終了要求の入力                          |
| 強制電源遮断操作部    | 電源部       | 常時給電側の回路による直接経路               | 通常終了できない場合の電源強制遮断                       |
| 緊急停止操作部      | 制御部       | GPIO/IRQ（通常閉じているNC接点による状態検出）      | 緊急停止操作状態の検出                             |
| 緊急停止操作部      | 走行駆動系     | NC接点による独立したハードウェア遮断経路              | 通常の制御ソフトウェアおよび通常通信に依存しない走行駆動出力の禁止または無効化 |
| 制御部          | 電源部       | GPIOによる有効化制御＋電源正常（PG）／異常（FLT）等の通知             | 電源状態の取得、電源保持および電源遮断制御                   |
| 電源部          | 本体各部      | 電源供給                         | 本体各部への電力供給                              |
| 保守用インターフェース  | 制御部       | 3.3 V UART                    | 本体へ直接接続した診断情報およびログの参照                   |

AONは、主バッテリー接続中に、主電源OFF時にも必要な回路への給電を維持する系統を指す。電源制御、緊急停止接点および信号の詳細は、電源・ハードウェア基本設計の6章、10章および19章に従う。
...
| 時刻管理 | RTC | RA8M2内蔵RTC＋32.768 kHz SOSC | 一部確定 | `PWR_AON`由来の電源とRTCバックアップ電池からVBATTを保持する。通常は`PWR_AON`側を優先する。水晶振動子・回路は未確定 |
| 時刻管理 | RTCバックアップ電池 | 交換可能な3 Vコイン形一次電池（CR2032クラス） | 一部確定 | RTC/VBATT保持専用。主バッテリー側の保持電源喪失時にもRTCを保持する。具体型式・ホルダ・切替え回路は未確定 |
...
RTCの二つの保持電源、電源切替えおよび時刻の信頼性確認は、「RTC継続性・時刻復旧契約基本設計」の2章・5章に従う。バックアップ電池はRTC/VBATT保持専用であり、本体の通常動作・通信・アクチュエータへの給電には使用しない。
