...
| 13  | BLE接続待機状態             | 低消費電力状態              | `otaSettlementPending=false`であること。電源・省電力管理機能から、移行識別子および縮退情報を含む低消費電力移行コミット可能結果と、選択済みMCU低消費電力モードを取得していること。選択理由、必要な監視の維持、他の必須条件、移行識別子およびより優先度の高い要求がないことを確認して、コミットを許可すること。低消費電力状態への更新、状態移行処理中情報の解除、電源・省電力管理機能による移行正常完了結果の確定、およびRA8M2の選択済みモードへの移行を不可分に実行し、コミットが成立していること |
...
BLE接続待機状態から低消費電力状態への遷移では、電源・省電力管理機能が事前処理の完了後に低消費電力移行コミット可能結果を提供する。
システム状態管理機能は、この結果、他の必須条件および移行識別子を確認する。
さらに、より優先度の高い要求がないことを確認した上で、電源・省電力管理機能へコミットを許可する。
コミット可能結果は、低消費電力移行の正常完了結果とは扱わない。

コミット許可後は、一つのクリティカル区間内で、次の処理を連続して実行する。
...
選択済みモードへ移行できず、定義された復帰要因も成立していない場合は、同じクリティカル区間内で現在状態の更新と移行正常完了結果を取り消す。
状態移行処理中を示す情報の解除も取り消し、状態移行処理中としての管理を継続したまま失敗処理へ引き継ぐ。この場合、他機能へ低消費電力状態を提供せず、低消費電力状態を有効にしない。
...
| 13                      | BLE接続待機状態から低消費電力状態への遷移                           | 低消費電力移行コミット可能結果または有効な選択済みMCU低消費電力モードを取得できない場合、コミット許可前に必須条件が不成立となった場合、または選択済みモードへの移行を含むコミットが成立しない場合は、低消費電力状態を有効にしない。コミット用クリティカル区間内で状態を一時更新していた場合は、他機能へ状態変更結果を提供する前に現在状態の更新と移行正常完了結果を取り消す。状態移行処理中を示す情報の解除も取り消し、状態移行処理中としての管理を継続したまま失敗処理へ引き継ぐ。BLE接続待機状態を安全に維持できる場合は、電源・省電力管理機能へ復元要求を出し、復元正常完了結果を取得した後にBLE接続待機状態を維持する。安全に維持できない場合は異常管理機能と連携して安全側の処理へ移行する |
